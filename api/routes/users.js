...
router.get('/', async function (request, response) {
  try {
    const data = await db.any('SELECT id, first_name, last_name, email, photo_url FROM users')
...
router.get('/:id', async function (request, response) {
  try {
    ...
    const data = await db.any(`SELECT id, first_name, last_name, email, photo_url FROM users WHERE id=${getUser}`)
    return response.json({
      data: data
    })
  } catch (err) {
